...
.PARAMETER Id
The asset ID of the Cloud Asset to set criticality for.
...
#>
function Set-CriticalityCloudAsset {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Mandatory = $false)]
        [Decimal]
    ...
    Process {
        'Calling method: Set-CriticalityCloudAsset' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug
        ...
        $LocalVarUri = '/api/client/assets/cloudAsset/show/{id}/criticality'
        if (!$Id) {
            throw "Error! The required parameter `Id` missing when calling setCriticalityCloudAsset."
        }
        $LocalVarUri = $LocalVarUri.replace('{id}', [System.Web.HTTPUtility]::UrlEncode($Id))

        if (!$SetCriticalityDto) {
            throw "Error! The required parameter `SetCriticalityDto` missing when calling setCriticalityCloudAsset."
        }
