...
[**Get-AssetSubdomainChangelog**](SubdomainsApi.md#Get-AssetSubdomainChangelog) | **GET** /api/client/assets/subdomain/show/{id}/changelog | Get Subdomain Changelog
...
# Get Subdomain Changelog
try {
    $Result = Get-AssetSubdomainChangelog -Id $Id -Page $Page -PageSize $PageSize
} catch {
    Write-Host ("Exception occurred when calling Get-AssetSubdomainChangelog: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```
...
$IncludeDnsRecords = $true # Boolean | When `true`, include up to 5 DNS records per asset inline in each item's `dns_records` field. Defaults to `false` to keep list responses fast; use `GET /assets/subdomain/show/{id}/dns-records` for the full paginated set. (optional)
 ...
 **IncludeDnsRecords** | **Boolean**| When &#x60;true&#x60;, include up to 5 DNS records per asset inline in each item&#39;s &#x60;dns_records&#x60; field. Defaults to &#x60;false&#x60; to keep list responses fast; use &#x60;GET /assets/subdomain/show/{id}/dns-records&#x60; for the full paginated set. | [optional] 
